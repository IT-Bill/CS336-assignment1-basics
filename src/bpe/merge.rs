...
fn id_pair_to_bytes_pair(
    symbol_bytes: &[Vec<u8>],
    id_pair: (usize, usize),
) -> (Vec<u8>, Vec<u8>) {
    (
        symbol_bytes[id_pair.0].clone(),
        symbol_bytes[id_pair.1].clone(),
    )
}

pub fn merge_impl(
    token_count: &HashMap<Vec<u8>, u32>,
    num_merges: usize,
    ...
    let pb = create_progress_bar(token_count.len() as u64, "Building tokens");

    for (token_idx, (sym_ids, count)) in token_count.iter().enumerate().progress_with(pb.clone()) {
    ...
    let pb = create_progress_bar(num_merges as u64, "Merging");

    for _ in (0..num_merges).progress_with(pb.clone()) {
        ...
        let new_symbol_bytes = [&symbol_bytes[sym_a_id][..], &symbol_bytes[sym_b_id][..]].concat();
        ...
        for (occ_idx, (token_idx, sym_idx)) in occurrences.iter().enumerate() {
            ...
            if let Some(l_idx) = l_idx {
                ...
                *pair_count.entry(la_id).or_default() -= token.count;
                pair_count_heap.push((
                    pair_count[&la_id],
                    id_pair_to_bytes_pair(&symbol_bytes, la_id),
                    la_id,
                ));
                ...
                *pair_count.entry(lab_id).or_default() += token.count;
                pair_occurrences
                    .entry(lab_id)
                    .or_default()
                    .push((*token_idx, l_idx));
                pair_count_heap.push((
                    pair_count[&lab_id],
                    id_pair_to_bytes_pair(&symbol_bytes, lab_id),
                    lab_id,
                ));
            }
            ...
            if let Some(r_idx) = r_idx {
                ...
                *pair_count.entry(br_id).or_default() -= token.count;
                pair_count_heap.push((
                    pair_count[&br_id],
                    id_pair_to_bytes_pair(&symbol_bytes, br_id),
                    br_id,
                ));
                ...
                *pair_count.entry(abr_id).or_default() += token.count;
                ...
                pair_count_heap.push((
                    pair_count[&abr_id],
                    id_pair_to_bytes_pair(&symbol_bytes, abr_id),
                    abr_id,
                ));
            }
        }

        let kept_occurrences: Vec<(usize, usize)> =
            valid_occ_indices.iter().map(|&i| occurrences[i]).collect();
...
pub fn merge(
    ...
    Ok(merge_impl(&token_count, num_merges)?)
}
